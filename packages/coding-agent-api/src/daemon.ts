...
async function tryConnect(socketPath: string): Promise<DaemonClient | undefined> {
	...
	try {
		...
		if (hello.protocol.name !== DAEMON_PROTOCOL_NAME) {
			process.stderr.write(`prime-agent-api: unexpected daemon protocol ${hello.protocol.name}\n`);
			client.close();
			return undefined;
		}
		return client;
...
export async function connectDaemon(): Promise<DaemonClient> {
	const socketPath = process.env.PRIME_AGENT_API_DAEMON_SOCKET || process.env.PRIME_AGENT_WEB_DAEMON_SOCKET || defaultDaemonSocketPath();
